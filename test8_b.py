...
#            cv2.imwrite('coba/charas/'+nm_fl+'-'+str(cou)+'-'+str(h)+'-'+str(w)+'.jpg',img[y:y+h,x:x+w])
    ...
    for ind in sorted(the_charas.keys()):
        temp = the_charas[ind]
        w,h = temp[1]
        y = temp[0]
        x = ind
...
#    img = pre.imageToBinary(redefine={'flag':True,'img':img},mode='inverse',alg='global')
    plate_cascade = cv2.CascadeClassifier('D:\\05111540000055_PBaskara\\src\\resources\\CascadeClassifier\cascadeRGB_v2.xml')
    
    detected = plate_cascade.detectMultiScale(possibleROI)
...
#        sift = cv2.ORB_create()
    ...
    #            print(n.imgIdx)
        ...
        if len(detected_points) <= 3:
            ...
            print("Template : "+str(ind)+'.jpg')
            afterHaar,pts = haarCascade(bgr,flag=True,point=detected_points[ int(len(detected_points)/2 - 1) ])#doHough(img,detected_points[0],bgr)
            break
...
#        templates.append(cv2.imread('resources/PositiveNew_v2/proc/imgRef/'+str(i)+'.jpg',0))
    ...
    print(templates[0].shape,len(templates))
